<?include 'include/headerInner.php';?>
	
	<ul>
		<li><a href="about.php">О заводе</a></li>
		<li><a href="analog_table.php">Аналоги</a></li>
		<li><a href="catalog.php">Каталог</a></li>
		<li><a href="catalogInner.php">Каталог внутренняя</a></li>
		<li><a href="ceni.php">Цены</a></li>
		<li><a href="contacts.php">Контакты</a></li>
		<li><a href="docs.php">Документы</a></li>
		<li><a href="main.php">Главная</a></li>
		<li><a href="news.php">Новости</a></li>
		<li><a href="otdel.php">Отдел</a></li>
		<li><a href="otzivy.php">Отзывы</a></li>
		<li><a href="sertificats.php">Сертификаты</a></li>
		<li><a href="service.php">Сервис</a></li>
		<li><a href="tovar.php">Товар</a></li>
		<li><a href="vacancies.php">Вакансии</a></li>
		<li><a href="video.php">Видеоинструкции</a></li>
		<li><a href="vistavki.php">Выставки</a></li>
	</ul>

<?include 'include/footer.php';?>